...
export async function requireUser(req, res, body, userIdFields = []) {
  ...
  if (bodyUserId && String(bodyUserId) !== data.user.id) {
    sendJson(res, 403, { success: false, error: 'This request does not belong to the logged-in user' })
    return null
  }
...
export async function forwardToN8n(req, res, { webhookEnvName, userIdFields = [], afterSuccess }) {
  ...
  try {
    ...
    sendJson(res, 502, { success: false, error: error instanceof Error ? error.message : 'Webhook request failed' })
  }
}
