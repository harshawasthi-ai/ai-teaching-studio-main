    ...
    ;(async () => {
      ...
      if (profile.role === 'teacher') {
        ...
        if (lessonResult.error || submissionResult.error || studentResult.error) {
          toast.error(lessonResult.error?.message || submissionResult.error?.message || studentResult.error?.message || 'Unable to load analytics')
          setState('error')
          return
        }
      ...
      if (lessonResult.error || submissionResult.error || quizResult.error) {
        toast.error(lessonResult.error?.message || submissionResult.error?.message || quizResult.error?.message || 'Unable to load analytics')
        setState('error')
        return
      }
      setLessons((lessonResult.data || []) as LessonRow[])
  ...
  return (
    <div className="min-h-screen relative">
      ...
      {state === 'loading' ? (
        ...
        <TeacherAnalytics lessons={lessons} submissions={submissions} studentProfiles={studentProfiles} />
      ) : (
...
function TeacherAnalytics({
  ...
  const filteredLessonIds = useMemo(() => new Set(filteredLessons.map(lesson => lesson.id)), [filteredLessons])
  ...
  const homeworkScores = filteredSubmissions.map(s => s.ai_homework_understanding_score).filter(isNumber)
  ...
  const lessonRows = useMemo(() => {
    return filteredLessons.map(lesson => {
      const lessonSubmissions = filteredSubmissions.filter(submission => submission.lesson_id === lesson.id)
      return {
        id: lesson.id,
        ...
        homework: average(lessonSubmissions.map(s => s.ai_homework_understanding_score).filter(isNumber)),
      }
    }).filter(row => row.submissions > 0 || filteredLessons.length <= 6).slice(0, 8)
  }, [filteredLessons, filteredSubmissions])
  const lessonsWithSubmissions = useMemo(() => {
    return filteredLessons.filter(lesson => filteredSubmissions.some(submission => submission.lesson_id === lesson.id))
  }, [filteredLessons, filteredSubmissions])
  useEffect(() => {
    if (selectedHotspotLessonId && !lessonsWithSubmissions.some(lesson => lesson.id === selectedHotspotLessonId)) {
  ...
  const selectedLessonSubmissions = useMemo(() => {
    ...
    return filteredSubmissions.filter(submission => submission.lesson_id === selectedHotspotLessonId)
  ...
  const studentsNeedingAttention = useMemo(() => buildAtRiskStudents(filteredSubmissions), [filteredSubmissions])
  ...
  const homeworkUnderstanding = useMemo(() => buildHomeworkUnderstanding(filteredSubmissions), [filteredSubmissions])
  const studentRows = useMemo(() => buildStudentAnalyticsRows(filteredSubmissions, lessonById), [filteredSubmissions, lessonById])
  ...
  const strugglingTopics = useMemo(() => buildStrugglingTopics(filteredLessons, filteredSubmissions), [filteredLessons, filteredSubmissions])
  ...
  return (
    <main className="relative max-w-6xl mx-auto px-4 sm:px-6 py-10">
      <DashboardHeader
        ...
        actions={(
          <>
            <button type="button" onClick={() => navigate({ to: '/submissions' })} className="dashboard-action">
              <ClipboardCheck className="w-4 h-4" /> Submissions
            </button>
            <button type="button" onClick={() => navigate({ to: '/library' })} className="dashboard-action">
      ...
      <section className="rounded-3xl border border-white/10 p-4 sm:p-5 mb-6" style={{ background: 'rgba(255,255,255,0.06)' }}>
        <div className="grid md:grid-cols-4 gap-3 items-end">
          <AnalyticsFilter label="Grade" value={gradeFilter} onChange={setGradeFilter} options={grades} allLabel="All grades" />
          <AnalyticsFilter label="Subject" value={subjectFilter} onChange={setSubjectFilter} options={subjects} allLabel="All subjects" />
          <label className="block">
            ...
            >
              ...
              {lessons
                .filter(lesson => (gradeFilter === 'all' || lesson.grade === gradeFilter) && (subjectFilter === 'all' || lesson.subject === subjectFilter))
                .map(lesson => (
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <MetricCard icon={<BookOpen />} label="Lessons" value={filteredLessons.length} detail={`${publishedLessons} published`} />
        <MetricCard icon={<Users />} label="Students Reached" value={uniqueStudents} detail="With submitted work" />
        <MetricCard icon={<Target />} label="Avg Worksheet" value={formatScore(avgWorksheet)} detail="AI worksheet marks" />
        <MetricCard icon={<TrendingUp />} label="Avg Homework" value={formatScore(avgHomework, '/5')} detail="Understanding score" />
      </div>
      ...
      <div className="grid lg:grid-cols-[1.4fr_1fr] gap-5">
        <ChartPanel title="Lesson Performance" subtitle="Average scores by lesson">
          <ResponsiveContainer width="100%" height={260}>
            <BarChart data={lessonRows}>
              ...
              <XAxis dataKey="name" tick={{ fill: '#bfdbfe', fontSize: 11 }} interval={0} angle={-12} height={70} />
        ...
        <ChartPanel
          ...
          subtitle={selectedHotspotLesson ? `Only for ${titleCase(selectedHotspotLesson.topic)}` : 'Choose a lesson to view question-level insight'}
        >
          <select
          ...
          <div className="space-y-3">
            {questionInsights.length === 0 ? (
              ...
              <div key={item.q} className="rounded-2xl border border-white/10 bg-slate-950/20 p-3">
                ...
                <div className="mt-2 h-2 rounded-full bg-white/10 overflow-hidden">
                  <div className="h-full rounded-full bg-cyan-400" style={{ width: `${item.averagePercent}%` }} />
                </div>
            ...
            <div key={item.level} className="dashboard-list-row">
              <div className="min-w-0 flex-1">
                <div className="flex items-center justify-between gap-3 text-sm">
                  ...
                  <span className="text-blue-200">{item.count} student{item.count === 1 ? '' : 's'}</span>
                </div>
                <div className="mt-2 h-2 rounded-full bg-white/10 overflow-hidden">
      ...
      <section className="rounded-3xl border border-white/10 p-5 mt-5" style={{ background: 'rgba(255,255,255,0.06)' }}>
                    ...
                    <>
                      <tr key={student.id} className="text-sm">
                        <td className="rounded-l-2xl border-y border-l border-white/10 bg-slate-950/20 px-4 py-3">
                          ...
                          >
                            <ChevronDown className={`w-4 h-4 text-blue-200 transition ${isExpanded ? 'rotate-180' : ''}`} />
                            <span>
                              <span className="block text-white font-semibold">{student.name}</span>
                              <span className="block text-blue-200/70 text-xs">{student.grade}</span>
                            </span>
                          </button>
                        </td>
                        <td className="border-y border-white/10 bg-slate-950/20 px-4 py-3 text-white/80">{student.submissions}</td>
                        ...
                        <td className="border-y border-white/10 bg-slate-950/20 px-4 py-3 text-blue-100">{formatDate(student.latestActivity)}</td>
                        <td className="rounded-r-2xl border-y border-r border-white/10 bg-slate-950/20 px-4 py-3">
                          <span className={`rounded-full border px-3 py-1 text-xs ${student.weakSignal === 'On track' ? 'border-emerald-300/30 bg-emerald-400/10 text-emerald-100' : 'border-amber-300/30 bg-amber-400/10 text-amber-100'}`}>
                            {student.weakSignal}
                          </span>
                        </td>
                      </tr>
                      {isExpanded && (
                        <tr key={`${student.id}-details`}>
                          <td colSpan={6} className="rounded-2xl border border-blue-300/15 bg-blue-500/5 px-4 py-4">
...
function StudentAnalytics({
  ...
  const subjects = useMemo(() => uniqueSorted(lessons.map(lesson => lesson.subject)), [lessons])
  const filteredLessons = useMemo(() => {
    return subjectFilter === 'all' ? lessons : lessons.filter(lesson => lesson.subject === subjectFilter)
  ...
  const lessonById = useMemo(() => new Map(filteredLessons.map(lesson => [lesson.id, lesson])), [filteredLessons])
  const worksheetScores = filteredSubmissions.map(s => s.ai_worksheet_score).filter(isNumber)
  const homeworkScores = filteredSubmissions.map(s => s.ai_homework_understanding_score).filter(isNumber)
  ...
  const quizAnsweredCount = filteredQuizAttempts.reduce((sum, attempt) => sum + Object.values(attempt.answers || {}).filter(Boolean).length, 0)
  ...
  const trend = filteredSubmissions
    ...
    .map((submission, index) => ({
      name: lessonById.get(submission.lesson_id)?.topic ? titleCase(lessonById.get(submission.lesson_id)!.topic).slice(0, 16) : `Lesson ${index + 1}`,
  ...
  return (
    <main className="relative max-w-6xl mx-auto px-4 sm:px-6 py-10">
      <DashboardHeader
        ...
        actions={(
          <button type="button" onClick={() => navigate({ to: '/student' })} className="dashboard-action">
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <MetricCard icon={<BookOpen />} label="Assigned Lessons" value={filteredLessons.length} detail={`${completedLessons} opened/submitted`} />
        <MetricCard icon={<ClipboardCheck />} label="Worksheets" value={worksheetSubmitted} detail="Submitted" />
        <MetricCard icon={<CheckCircle2 />} label="Homework" value={homeworkSubmitted} detail="Submitted" />
        <MetricCard icon={<Target />} label="Quiz Practice" value={quizAnsweredCount} detail="MCQs answered" />
      </div>
      ...
      <div className="grid lg:grid-cols-[1.3fr_1fr] gap-5">
          ...
          {trend.length === 0 ? (
            ...
            <ResponsiveContainer width="100%" height={260}>
              <LineChart data={trend}>
                ...
                <YAxis tick={{ fill: '#bfdbfe', fontSize: 12 }} />
                <Tooltip contentStyle={tooltipStyle} />
                <Line type="monotone" dataKey="score" stroke="#38bdf8" strokeWidth={3} dot={{ r: 5 }} name="Score" />
              </LineChart>
            </ResponsiveContainer>
          )}
        </ChartPanel>
        ...
        <InsightPanel title="Your Averages">
          ...
          <AverageRow label="Homework understanding" value={formatScore(average(homeworkScores), '/5')} />
          <AverageRow label="Lessons completed" value={`${completedLessons}/${filteredLessons.length || 0}`} />
      ...
      <div className="grid lg:grid-cols-2 gap-5 mt-5">
        <TopicPerformancePanel title="Topics You Are Performing Good In" rows={topicPerformance.good} tone="good" />
        <TopicPerformancePanel title="Topics That Need Improvement" rows={topicPerformance.needsImprovement} tone="needs" />
      </div>
      ...
      <div className="grid lg:grid-cols-2 gap-5 mt-5">
        <InsightPanel title="Recent Feedback">
          {filteredSubmissions.slice(0, 5).map(submission => {
            ...
            return (
              <div key={submission.id} className="rounded-2xl border border-white/10 bg-slate-950/20 p-4">
                <div className="flex items-center justify-between gap-3">
                  <p className="text-white font-semibold">{lesson ? titleCase(lesson.topic) : 'Lesson'}</p>
                  <span className="text-xs text-blue-200">{formatDate(submission.updated_at || submission.submitted_at)}</span>
                </div>
                <p className="text-white/75 text-sm mt-2">{feedback || 'Submitted. Feedback will appear after evaluation.'}</p>
              </div>
            )
          })}
          {filteredSubmissions.length === 0 && <EmptyInsight text="Feedback will appear after you submit worksheet or homework." />}
        </InsightPanel>
...
function MetricCard({ icon, label, value, detail }: { icon: React.ReactElement; label: string; value: string | number; detail: string }) {
  return (
    <section className="rounded-3xl border border-white/10 p-5" style={{ background: 'rgba(255,255,255,0.06)' }}>
      <div className="flex items-center justify-between gap-3">
...
function PendingSubmissionsPanel({
  ...
  const pendingCount = rows.reduce((sum, row) => sum + row.missingWorksheet.length + row.missingHomework.length, 0)
  return (
    <section className="rounded-3xl border border-white/10 p-5 mb-6" style={{ background: 'rgba(255,255,255,0.06)' }}>
      <div className="flex flex-wrap items-start justify-between gap-3 mb-4">
        <div>
          <h2 className="text-white text-xl font-bold">Pending Submissions</h2>
          <p className="text-blue-300 text-sm mt-1">Published lessons where students still need to submit worksheet or homework.</p>
        </div>
      ...
      {rows.length === 0 ? (
        ...
        <div className="grid lg:grid-cols-2 gap-4">
          {rows.slice(0, 6).map(row => (
            <div key={row.lessonId} className="rounded-2xl border border-white/10 bg-slate-950/20 p-4">
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div>
                  <p className="text-white font-bold">{row.lessonTitle}</p>
                  <p className="text-blue-200 text-sm mt-1">{row.grade} · {row.totalStudents} student{row.totalStudents === 1 ? '' : 's'}</p>
                </div>
    ...
    <section className="rounded-3xl border border-white/10 p-5 mb-6" style={{ background: 'rgba(255,255,255,0.06)' }}>
      <div className="flex flex-wrap items-start justify-between gap-3 mb-4">
        <div>
          ...
          <p className="text-blue-300 text-sm mt-1">Calculated from the lowest average worksheet marks by lesson.</p>
        </div>
            ...
            <div key={row.lessonId} className="rounded-2xl border border-white/10 bg-slate-950/20 p-4">
              <div className="flex items-start justify-between gap-3">
                <div>
                  ...
                  <p className="text-blue-200 text-sm mt-1">{row.subject} · {row.grade}</p>
                </div>
              ...
              <div className="mt-4 flex items-end justify-between gap-3">
                ...
                <p className="text-white/65 text-sm">{row.submissions} submission{row.submissions === 1 ? '' : 's'}</p>
              </div>
              <div className="mt-4 h-2 rounded-full bg-white/10 overflow-hidden">
                <div
                  className={`h-full rounded-full ${row.level === 'High' ? 'bg-red-400' : row.level === 'Medium' ? 'bg-amber-400' : 'bg-blue-400'}`}
                  style={{ width: `${Math.min(100, Math.max(0, ((row.averageScore || 0) / 10) * 100))}%` }}
                />
              </div>
            </div>
          ))}
      ...
      {students.length === 0 ? (
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          {students.slice(0, 4).map(student => (
            <span key={student.id} className="rounded-full border border-amber-300/20 bg-amber-400/10 px-2.5 py-1 text-xs text-amber-100">
...
function TopicPerformancePanel({
  ...
  return (
    <InsightPanel title={title}>
      {rows.length === 0 ? (
        <EmptyInsight text={good ? 'Strong topics will appear after your work is evaluated.' : 'Improvement topics will appear after your work is evaluated.'} />
...
function EmptyInsight({ text }: { text: string }) {
  return <p className="rounded-2xl border border-white/10 bg-slate-950/20 p-4 text-sm text-white/60">{text}</p>
}
...
function ScorePill({ value, suffix = '' }: { value: number | null; suffix?: string }) {
  ...
  return (
    <span className={`inline-flex rounded-full border px-3 py-1 text-xs font-semibold ${isWeak ? 'border-amber-300/30 bg-amber-400/10 text-amber-100' : 'border-blue-300/30 bg-blue-400/10 text-blue-100'}`}>
...
function StudentLessonBreakdown({
  ...
  rows: Array<{
    ...
    weakQuestions: Array<{ q: string; averagePercent: number; questionText: string; feedback?: string }>
        ...
        <div key={row.id} className="rounded-2xl border border-white/10 bg-slate-950/20 p-4">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              ...
              <p className="text-blue-200 text-xs mt-1">Latest activity {formatDate(row.latestActivity)}</p>
            </div>
            <div className="flex flex-wrap gap-2">
              <ScorePill value={row.worksheetScore} />
              <ScorePill value={row.homeworkScore} suffix="/5" />
          ...
          <div className="mt-3 grid md:grid-cols-2 gap-3">
            <div className="rounded-xl border border-white/10 bg-white/5 p-3">
              ...
              <p className="text-white/75 text-sm">{row.worksheetFeedback || 'No worksheet feedback yet.'}</p>
            </div>
            <div className="rounded-xl border border-white/10 bg-white/5 p-3">
              <p className="text-blue-200 text-xs font-semibold mb-2">Homework Feedback</p>
              <p className="text-white/75 text-sm">{row.homeworkFeedback || 'No homework feedback yet.'}</p>
            </div>
          </div>
...
function GuidanceItem({ text, tone }: { text: string; tone: 'good' | 'needs' }) {
  const good = tone === 'good'
  return (
    <div className={`flex items-start gap-3 rounded-2xl border p-4 ${good ? 'border-emerald-300/20 bg-emerald-400/10' : 'border-amber-300/20 bg-amber-400/10'}`}>
      <CheckCircle2 className={`w-5 h-5 shrink-0 mt-0.5 ${good ? 'text-emerald-300' : 'text-amber-300'}`} />
...
function buildAtRiskStudents(submissions: SubmissionRow[]) {
  ...
  return Array.from(byStudent.entries()).map(([id, rows]) => {
    const worksheetAvg = average(rows.map(row => row.ai_worksheet_score).filter(isNumber))
    const homeworkAvg = average(rows.map(row => row.ai_homework_understanding_score).filter(isNumber))
    ...
    return null
  }).filter(Boolean).slice(0, 6) as Array<{ id: string; name: string; score: string; reason: string }>
}

function buildStudentAnalyticsRows(submissions: SubmissionRow[], lessonById: Map<string, LessonRow>) {
  const byStudent = new Map<string, SubmissionRow[]>()
  submissions.forEach(submission => {
    const list = byStudent.get(submission.student_id) || []
    list.push(submission)
    ...
    .map(([id, rows]) => {
      ...
      const lessonRows = rows
        ...
        .map(row => {
          ...
          return {
            id: row.id,
            lessonTitle: lesson ? `${titleCase(lesson.topic)} - ${lesson.grade}` : row.grade || 'Lesson',
            ...
            weakQuestions: buildQuestionInsights([row], questionText).filter(item => item.averagePercent < 60),
...
function buildPendingSubmissions(
  ...
  return lessons
    ...
    .filter(row => row.totalStudents > 0 && (row.missingWorksheet.length > 0 || row.missingHomework.length > 0))
    .sort((a, b) => (b.missingWorksheet.length + b.missingHomework.length) - (a.missingWorksheet.length + a.missingHomework.length))
}

function buildStrugglingTopics(lessons: LessonRow[], submissions: SubmissionRow[]) {
  return lessons
    .map(lesson => {
      const lessonSubmissions = submissions.filter(submission => submission.lesson_id === lesson.id)
      const worksheetScores = lessonSubmissions.map(submission => submission.ai_worksheet_score).filter(isNumber)
...
function buildQuestionInsights(submissions: SubmissionRow[], questionTextByNumber: Map<string, string>) {
  ...
  return Array.from(byQuestion.entries())
    .map(([q, scores]) => {
      ...
      return {
        ...
        averagePercent: Math.round(average(scores.map(score => (score.awarded / score.max) * 100)) || 0),
      }
    })
...
function buildStudentGuidance(submissions: SubmissionRow[], lessonById: Map<string, LessonRow>, assignedLessonCount: number) {
  ...
  submissions.forEach(submission => {
    ...
    buildQuestionInsights([submission], questionText)
      ...
      .slice(0, 2)
      .forEach(item => {
        weakAreas.push(item.questionText ? `Practice Q${item.q}: ${item.questionText}` : `Practice worksheet question Q${item.q} in ${lessonTitle}.`)
      })
  })

  const submittedWorksheets = submissions.filter(submission => submission.worksheet_submitted_at).length
  const submittedHomework = submissions.filter(submission => submission.homework_submitted_at).length
...
function buildStudentTopicPerformance(submissions: SubmissionRow[], lessonById: Map<string, LessonRow>) {
