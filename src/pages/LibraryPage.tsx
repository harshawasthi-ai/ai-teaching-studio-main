...
import { Search, Trash2, Sparkles, X, Loader2, Send, ClipboardCheck, Lock, BarChart3 } from 'lucide-react'
...
interface LessonRow {
  ...
  duration: string; language: string; created_at: string
...
export default function LibraryPage() {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      const { data, error } = await supabase
        ...
        .limit(50)
      if (error) { toast.error(`❌ ${error.message}`); setLessons([]); return }
      setLessons(data || [])
    })()
  }, [user, authLoading])
  ...
  const filtered = useMemo(() => {
    ...
    const result = lessons.filter(l => {
      const matchesSearch = !q || [l.subject, l.grade, l.topic].some(f => f?.toLowerCase().includes(q))
  ...
  const filtersActive = Boolean(query.trim()) || subjectFilter !== 'all' || gradeFilter !== 'all' || sortMode !== 'newest'
  const resetFilters = () => {
    setQuery('')
  ...
  const deleteLesson = async () => {
    ...
    if (!user) return
    setDeleting(true)
    const { error } = await supabase.from('lessons').delete().eq('id', deleteTarget.id).eq('user_id', user.id)
    setDeleting(false)
  ...
  const togglePublished = async (lesson: LessonRow, e: React.MouseEvent) => {
    ...
    setLessons(current => current?.map(item => item.id === lesson.id
      ? { ...item, is_published: true, published_at: publishedAt }
      : item
    ) ?? [])
    ...
    <div className="min-h-screen relative">
        ...
        <div className="max-w-6xl mx-auto flex flex-wrap items-center justify-between gap-6">
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <button
              ...
              style={{ background: 'linear-gradient(135deg,#1E6FD9,#4DA3FF)', boxShadow: '0 8px 20px rgba(30,111,217,0.4)' }}
            >
      ...
      <main className="relative max-w-6xl mx-auto px-4 sm:px-6 py-8">
        <div className="relative mb-8">
          ...
          <input value={query} onChange={e => setQuery(e.target.value)} placeholder="Search by subject, grade or topic..."
        ...
        {lessons === null ? (
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {Array.from({ length: 6 }).map((_, i) => (
              <div key={i} className="h-48 rounded-2xl animate-pulse border border-white/10" style={{ background: 'rgba(255,255,255,0.06)' }} />
            ))}
          </div>
        ) : filtered.length === 0 ? (
          <div className="rounded-3xl p-12 text-center border border-white/10 backdrop-blur" style={{ background: 'rgba(255,255,255,0.06)' }}>
            <div className="text-6xl mb-4">📚</div>
            <h3 className="text-white font-bold text-xl">{query ? 'No matches' : 'No lessons yet'}</h3>
            <p className="text-blue-300 mt-1">{query ? 'Try a different search.' : 'Generate your first lesson kit'}</p>
            <button onClick={() => navigate({ to: '/' })} className="inline-flex items-center gap-2 mt-6 px-6 py-3 rounded-2xl text-white font-semibold"
              style={{ background: 'linear-gradient(135deg,#1E6FD9,#4DA3FF)', boxShadow: '0 8px 20px rgba(30,111,217,0.4)' }}>
          ...
          <>
            <div className="library-section-heading">
              <div>
                ...
                <p>{filtered.length} lesson kit{filtered.length === 1 ? '' : 's'} ready to open</p>
              </div>
            </div>
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {filtered.map(l => (
...
function uniqueOptions(lessons: LessonRow[] | null, key: 'subject' | 'grade') {
  return Array.from(new Set((lessons || []).map(lesson => lesson[key]).filter(Boolean))).sort((a, b) => a.localeCompare(b))
}
...
function LessonCard({
  ...
  const date = new Date(lesson.created_at).toLocaleDateString(undefined, { month: 'short', day: 'numeric', year: 'numeric' })
  return (
    <div onClick={onOpen} className="lesson-card relative rounded-2xl border border-white/10 backdrop-blur p-5 cursor-pointer hover:-translate-y-1 transition overflow-hidden">
      <div className={`absolute top-0 left-0 right-0 h-[3px] bg-gradient-to-r ${stripFor(lesson.subject)}`} />
      <button onClick={onDelete} aria-label="Delete" className="lesson-card-delete absolute top-3 right-3 text-white/30 hover:text-red-400 hover:bg-red-400/10 rounded-lg p-1.5 transition">
        <Trash2 className="w-4 h-4" />
      </button>
      <h3 className="text-white font-bold text-lg capitalize pr-8 mb-3">{lesson.topic}</h3>
      <div className="flex flex-wrap gap-1.5 mb-3">
        <span className="text-xs px-2.5 py-1 rounded-full bg-indigo-500/20 text-indigo-300 border border-indigo-400/30">{lesson.subject}</span>
        <span className="text-xs px-2.5 py-1 rounded-full bg-emerald-500/20 text-emerald-300 border border-emerald-400/30">{lesson.grade}</span>
        <span className="text-xs px-2.5 py-1 rounded-full bg-white/10 text-white/60">{lesson.duration}</span>
        {lesson.language === 'Hindi' && <span className="text-xs px-2.5 py-1 rounded-full bg-orange-500/20 text-orange-300">Hindi</span>}
      </div>
      <div className="text-xs text-blue-300/60 mb-4">{date}</div>
      <div className="grid grid-cols-2 gap-2">
        <button onClick={(e) => { e.stopPropagation(); onOpen() }} className="lesson-card-action py-2.5 rounded-xl text-white font-semibold text-sm hover:opacity-90">
          View Lesson
        </button>
        <button
          type="button"
        ...
        >
          {publishing ? <Loader2 className="w-4 h-4 animate-spin" /> : lesson.is_published ? <Lock className="w-4 h-4" /> : <Send className="w-4 h-4" />}
...
function DeleteLessonDialog({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
      ...
      />
      <div className="relative w-full max-w-md rounded-3xl border border-white/15 p-6 shadow-2xl" style={{ background: 'rgba(15,32,68,0.96)' }}>
        ...
        <p className="text-blue-200 text-sm mt-2">
          This will permanently remove <span className="text-white font-semibold">{lesson.topic}</span> from your library.
        </p>
