...
export default function SubmissionsPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setSubmissions(
        ((submissionResult.data || []) as SubmissionRow[]).filter(isTeacherVisibleSubmission),
      );
      setLoading(false);
    })();
  ...
  const saveAiEvaluation = async (
    ...
    toast.success(
      section === "worksheet"
        ? "✅ Worksheet AI evaluation updated"
        : "✅ Homework AI evaluation updated",
    );
  };
  ...
  return (
    <div className="min-h-screen relative">
      ...
      <main className="relative max-w-6xl mx-auto px-4 sm:px-6 py-10">
        <div className="flex flex-wrap items-center justify-between gap-4 mb-8">
          <div>
            ...
            <p className="text-blue-300 mt-2">
              Review worksheet answers and homework from your students.
            </p>
          </div>
          <div
            className="rounded-2xl border border-white/15 backdrop-blur px-5 py-3 text-center"
        ...
        <div
          className="rounded-3xl border border-white/10 p-4 sm:p-5 mb-8"
          style={{ background: "rgba(255,255,255,0.06)" }}
        >
        ...
        {loading ? (
          ...
          >
            ...
            <p className="text-blue-300 mt-2">
              Published lessons will appear here after students submit their work.
            </p>
          </div>
        ) : (
...
function isTeacherVisibleSubmission(submission: SubmissionRow) {
  ...
  const hasWorksheetAnswers = Object.values(submission.worksheet_answers || {}).some(
  ...
  return (
    ...
    >
      ...
      >
        ...
        <ChevronDown
          className={`mt-2 w-5 h-5 text-blue-200 transition ${expanded ? "rotate-180" : ""}`}
        />
      </button>
                ...
                <div className="space-y-3">
                  {answers
                    ...
                    .map(([questionNumber, answer]) => (
                      <div
                        key={questionNumber}
                        className="rounded-xl border border-white/10 bg-slate-950/20 px-3 py-2"
                      >
...
function EditableAiWorksheetEvaluation({
  ...
  const [score, setScore] = useState(
    submission.ai_worksheet_score == null ? "" : String(submission.ai_worksheet_score),
  );
  ...
  useEffect(() => {
    ...
    submission.ai_worksheet_feedback,
    submission.ai_worksheet_grade,
  ...
  return (
    <section className="mt-4 rounded-2xl border border-emerald-400/20 p-4 bg-emerald-500/10">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="text-emerald-100/80 text-xs mt-1">
            AI checked{" "}
            {submission.ai_worksheet_graded_at
              ? new Date(submission.ai_worksheet_graded_at).toLocaleString()
              : ""}
          </p>
        </div>
        <button
      ...
      {editing ? (
        <div className="mt-4 grid gap-3 md:grid-cols-[140px_140px_1fr]">
          <label className="block">
            ...
            <input
              ...
              className="w-full rounded-2xl border border-white/15 bg-slate-950/30 px-4 py-3 text-white outline-none focus:border-blue-300/60"
            />
          </label>
          <label className="block">
        ...
        <>
          <div className="mt-3 flex flex-wrap gap-2">
            {submission.ai_worksheet_score != null && (
              <span className="rounded-full border border-emerald-300/30 bg-emerald-500/15 px-4 py-1.5 text-sm font-semibold text-emerald-100">
          ...
          {submission.ai_worksheet_feedback && (
            <p className="mt-3 rounded-xl border border-white/10 bg-slate-950/20 p-3 text-white/85 whitespace-pre-wrap">
              {submission.ai_worksheet_feedback}
            </p>
          )}
        </>
      ...
      {Array.isArray(submission.ai_worksheet_breakdown) &&
        submission.ai_worksheet_breakdown.length > 0 && (
              ...
              <div key={index} className="rounded-xl border border-white/10 bg-slate-950/20 p-3">
                <div className="flex items-center justify-between gap-2 text-sm">
                  ...
                  <span className="text-emerald-200">
                    {String(item.marks_awarded ?? 0)} / {String(item.max_marks ?? 0)}
                  </span>
                </div>
                {item.feedback != null && (
                  <p className="mt-2 text-xs text-white/65 whitespace-pre-wrap">
...
function EditableAiHomeworkEvaluation({
  ...
  const [score, setScore] = useState(
    submission.ai_homework_understanding_score == null
      ? ""
      : String(submission.ai_homework_understanding_score),
  );
  ...
  const [improvements, setImprovements] = useState(
    (submission.ai_homework_improvements || []).join("\n"),
  );
  ...
  const handleSave = () => {
    ...
    if (
      ...
      (!Number.isFinite(normalizedScore) || normalizedScore < 1 || normalizedScore > 5)
    ) {
  ...
  return (
    <section className="mt-4 rounded-2xl border border-blue-300/20 p-4 bg-blue-500/10">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="text-blue-100/80 text-xs mt-1">
            AI evaluated{" "}
            {submission.ai_homework_evaluated_at
              ? new Date(submission.ai_homework_evaluated_at).toLocaleString()
              : ""}
          </p>
        </div>
        <button
      ...
      {editing ? (
        ...
        <>
          <div className="mt-3 flex flex-wrap gap-2">
            {submission.ai_homework_understanding_level && (
              <span className="rounded-full border border-blue-300/30 bg-blue-500/15 px-4 py-1.5 text-sm font-semibold text-blue-100">
                {submission.ai_homework_understanding_level}
              </span>
            )}
            {submission.ai_homework_understanding_score != null && (
              <span className="rounded-full border border-emerald-300/30 bg-emerald-500/15 px-4 py-1.5 text-sm font-semibold text-emerald-100">
                {submission.ai_homework_understanding_score} / 5
              </span>
            )}
          </div>
          {submission.ai_homework_feedback && (
            <p className="mt-3 rounded-xl border border-white/10 bg-slate-950/20 p-3 text-white/85 whitespace-pre-wrap">
              {submission.ai_homework_feedback}
            </p>
          )}
          ...
          {submission.ai_homework_teacher_note && (
            <p className="mt-3 rounded-xl border border-white/10 bg-slate-950/20 p-3 text-sm text-white/70 whitespace-pre-wrap">
              Teacher note: {submission.ai_homework_teacher_note}
            </p>
          )}
        </>
      )}
