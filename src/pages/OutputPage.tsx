...
import {
  evaluateHomeworkWithAi,
  gradeWorksheetWithAi,
  regenerateSection,
  regenerateWorksheetAnswer,
  saveLessonSection,
  saveLessonSections,
    ...
    if (this.state.hasError) {
      return (
        <div className="flex flex-col items-center justify-center min-h-screen text-white px-6 text-center">
          ...
          <p className="text-white/70 mb-8">An unexpected error occurred while loading the page.</p>
          <button
            onClick={() => {
...
function writeStudentQuizAnswers(
...
async function saveQuizPracticeAnswers({
  ...
  const { error } = await supabase.from("quiz_practice_attempts").upsert(
    {
      lesson_id: lessonId,
      student_id: userId,
      answers,
  ...
  return (
    <div className="min-h-screen relative">
      ...
      <main className="relative min-h-[70vh] flex items-center justify-center px-4">
        <div
          className="w-full max-w-md rounded-3xl border border-white/10 backdrop-blur-xl p-8 text-center"
...
function OutputPageInner() {
  ...
  const [studentReviews, setStudentReviews] = useState<StudentReviews>({
    worksheet: null,
    homework: null,
  });
  ...
  useEffect(() => {
    ...
    const loadLesson = async () => {
      ...
      if (lessonId && user?.id) {
        ...
        const { data, error } = isTeacher
          ? await supabase
              .from("lessons")
              .select(
                "id,subject,grade,topic,duration,language,objectives,created_at,is_published,published_at,lesson_plan,worksheet,quiz,answer_key",
              )
              .eq("id", lessonId)
              .eq("user_id", user.id)
              .single()
          : await supabase
              .from("lessons")
              .select(
                "id,user_id,subject,grade,topic,duration,language,objectives,created_at,is_published,published_at,lesson_plan,worksheet,quiz,answer_key",
              )
              .eq("id", lessonId)
        ...
        if (error || !data) {
          setLoadError(
            "We could not find this lesson. It may have been deleted, or you may not have access to it.",
          );
        ...
        if (!isTeacher) {
          ...
          if (!cancelled) {
            ...
            const nextQuizAnswers =
              Object.keys(savedQuizAnswers).length > 0
                ? savedQuizAnswers
                : Object.keys(legacyQuizAnswers).length > 0
                  ? legacyQuizAnswers
                  : storedQuizAnswers;
            setStudentWorksheetAnswers(
              (submission?.worksheet_answers as StudentWorksheetAnswers | null) ?? {},
            );
            ...
            const hasWorksheetAnswers = Object.values(
              (submission?.worksheet_answers as StudentWorksheetAnswers | null) ?? {},
            ).some(Boolean);
            setWorksheetSubmittedAt(
              submission?.worksheet_submitted_at ??
                (hasWorksheetAnswers ? (submission?.submitted_at ?? null) : null),
            );
            setHomeworkSubmittedAt(
              submission?.homework_submitted_at ??
                (submission?.homework_answer ? (submission?.submitted_at ?? null) : null),
            );
            setAiWorksheetReview(
              submission?.ai_worksheet_graded_at
                ? {
                    score:
                      typeof submission.ai_worksheet_score === "number"
                        ? submission.ai_worksheet_score
                        : null,
                    grade: submission.ai_worksheet_grade || "",
                    feedback: submission.ai_worksheet_feedback || "",
                    breakdown: Array.isArray(submission.ai_worksheet_breakdown)
                      ? (submission.ai_worksheet_breakdown as Array<Record<string, unknown>>)
                      : [],
            ...
            setAiHomeworkReview(
              submission?.ai_homework_evaluated_at
                ? {
                    ...
                    understanding_score:
                      typeof submission.ai_homework_understanding_score === "number"
                        ? submission.ai_homework_understanding_score
                        : null,
                    feedback: submission.ai_homework_feedback || "",
                    strengths: Array.isArray(submission.ai_homework_strengths)
                      ? submission.ai_homework_strengths.map(String)
                      : [],
                    improvements: Array.isArray(submission.ai_homework_improvements)
                      ? submission.ai_homework_improvements.map(String)
                      : [],
            ...
            setStudentReviews({
              worksheet:
                submission?.worksheet_reviewed_at || submission?.reviewed_at
                  ? {
                      score:
                        typeof submission.worksheet_score === "number"
                          ? submission.worksheet_score
                          : typeof submission.score === "number"
                            ? submission.score
                            : null,
                      feedback: submission.worksheet_feedback || submission.feedback || "",
                      reviewed_at:
                        submission.worksheet_reviewed_at || submission.reviewed_at || null,
                    }
                  : null,
              homework: submission?.homework_reviewed_at
                ? {
                    score:
                      typeof submission.homework_score === "number"
                        ? submission.homework_score
                        : null,
      ...
      if (!stored) {
        setLoadError(
          "No lesson is currently open. Create a new lesson or choose one from your library.",
        );
        setLoadingLesson(false);
        return;
      }
  ...
  const saveEdit = async () => {
    ...
    try {
      ...
      const sectionValue =
        tab === "homework" ? { ...(lesson.lesson_plan || {}), homework: draft } : draft;
      ...
      const next = (
        tab === "homework"
          ...
          : {
              ...
              id: lessonId,
              [tab]: freshSection,
              ...((tab === "worksheet" || tab === "quiz") && fresh.answer_key
                ? { answer_key: fresh.answer_key }
                : {}),
              metadata: { ...lesson.metadata, id: lessonId },
            }
  ...
  const handleRegenerateWorksheetQuestion = async (
    ...
    try {
      ...
      const freshGroup = collectWorksheetQuestions(fresh.worksheet).find(
        (group) => group.key === groupKey,
      );
      const freshQuestion = freshGroup?.questions[questionIndex];
      if (!freshQuestion)
        throw new Error("The regenerated response did not include a replacement question");

      const freshAnswer = findAnswerForQuestion(
        freshQuestion,
        fresh.answer_key?.worksheet_answers || [],
      );
      if (!freshAnswer)
        throw new Error(
          "The regenerated response did not include an answer for the replacement question",
        );

      const nextWorksheet = replaceWorksheetQuestion(
        lesson.worksheet,
        groupKey,
        questionIndex,
        question,
      ...
      toast.success(
        lessonId
          ? "✅ Question and answer regenerated!"
          : "✅ Question and answer regenerated for this session!",
      );
    } catch (e: unknown) {
      toast.error(`❌ ${getErrorMessage(e)}`);
    } finally {
  ...
  const handleRegenerateWorksheetAnswer = async (
    ...
    try {
      ...
      sessionStorage.setItem("currentLesson", JSON.stringify(next));
      toast.success(
        lessonId ? "✅ Answer regenerated!" : "✅ Answer regenerated for this session!",
      );
    } catch (e: unknown) {
      toast.error(`❌ ${getErrorMessage(e)}`);
    } finally {
    ...
    try {
      ...
      if (section === "homework") {
        ...
        try {
          ...
          const homeworkTask = String(
            lesson.lesson_plan?.homework?.description ||
              lesson.lesson_plan?.homework?.activity ||
              "",
          );
          ...
          toast.error(
            `❌ ${getErrorMessage(evaluationError, "Homework submitted, but AI evaluation failed")}`,
          );
        }
      }
      if (section === "worksheet") {
        ...
        try {
          ...
          toast.error(
            `❌ ${getErrorMessage(gradingError, "Worksheet submitted, but AI grading failed")}`,
          );
        }
      }
  ...
  const selectedExport =
    availableExportOptions.find((option) => option.key === exportMode) ?? availableExportOptions[0];
  const downloadPdf = (mode: ExportMode = exportMode) => {
    ...
    const option =
      availableExportOptions.find((item) => item.key === safeMode) ?? availableExportOptions[0];
    const topicSlug =
      String(m.topic || "lesson-kit")
        .trim()
        .replace(/[^a-z0-9]+/gi, "-")
        .replace(/^-|-$/g, "")
        .toLowerCase() || "lesson-kit";
      ...
      <main className="relative max-w-5xl mx-auto px-4 sm:px-6 py-8">
          ...
          <div className="relative">
            ...
            {exportMenuOpen && (
              <div className="pdf-export-menu absolute right-0 mt-2 w-56 rounded-2xl border p-2 shadow-2xl z-30">
                <div className="px-3 py-2 text-xs font-semibold uppercase tracking-wide text-blue-300">
                  Export
                </div>
        ...
        {lessonLocked && (
          <div className="mb-4 rounded-2xl border border-emerald-400/20 bg-emerald-500/10 px-4 py-3 text-sm text-emerald-200">
            This lesson is published and locked. Students can view it, but teacher edits and
        ...
        {regen ? (
          ...
          <div>
            {tab === "lesson_plan" && <LessonPlanView data={lesson.lesson_plan} />}
            {tab === "homework" && (
              <HomeworkView
            ...
            {!isTeacher && (tab === "worksheet" || tab === "homework") && (
              <>
                <StudentSubmissionPanel
                  ...
                  locked={
                    (tab === "homework" && Boolean(homeworkSubmittedAt)) ||
                    (tab === "worksheet" && Boolean(worksheetSubmittedAt))
                  }
                  onSubmit={() => submitStudentWork(tab)}
                />
                {tab === "worksheet" && aiWorksheetReview && (
                  <StudentAiWorksheetPanel review={aiWorksheetReview} />
                )}
                {tab === "homework" && aiHomeworkReview && (
                  <StudentAiHomeworkPanel review={aiHomeworkReview} />
                )}
            ...
            <div key={index} className="rounded-2xl border border-white/10 bg-slate-950/20 p-3">
              <div className="flex flex-wrap items-center justify-between gap-2 text-sm">
                <span className="font-semibold text-blue-200">
                  Q{String(item.q_number ?? index + 1)}
                </span>
...
function EditorShell({ title, children }: { title: string; children: ReactNode }) {
  return (
    <div
      className="rounded-3xl p-5 sm:p-6 border border-blue-400/30 backdrop-blur space-y-5"
      style={{ background: "rgba(30,111,217,0.12)" }}
    >
      <div>
        <h2 className="text-white font-bold text-xl">{title}</h2>
        <p className="text-blue-200 text-sm mt-1">
          Make changes here, then use Save Changes above.
        </p>
      </div>
      {children}
    </div>
...
function EditField({
  ...
  const commonClass =
    "w-full rounded-xl px-3 py-2.5 text-white placeholder-white/30 outline-none focus:border-blue-400 focus:ring-2 focus:ring-blue-400/20";
  return (
    <label className="block">
      <span className="text-xs font-semibold uppercase tracking-wide text-blue-200 mb-1.5 block">
        {label}
      </span>
      {rows > 1 ? (
        <textarea
...
function LessonPlanEditor({ data, onChange }: { data: EditableValue; onChange: EditableChange }) {
  const updateSection = (key: string, field: string, value: unknown) => {
    const section =
      typeof data[key] === "object" && data[key] !== null ? (data[key] as EditableValue) : {};
        ...
        return (
          ...
          >
            ...
            <EditField
              ...
              onChange={(v) => updateSection(key, "duration", v)}
            />
            <EditField
              label="Activity"
              rows={3}
              value={section.activity}
              onChange={(v) => updateSection(key, "activity", v)}
            />
            <EditField
              label="Content"
              rows={4}
              value={section.content}
              onChange={(v) => updateSection(key, "content", v)}
            />
            <EditField
              label="Description"
              rows={3}
            ...
            />
            <EditField
              label="Key Terms (one per line)"
              rows={3}
              value={arrayToLines(section.key_terms)}
              onChange={(v) => updateSection(key, "key_terms", linesToArray(v))}
            />
            <EditField
              label="Materials (one per line)"
              rows={3}
              value={arrayToLines(section.materials)}
              onChange={(v) => updateSection(key, "materials", linesToArray(v))}
            />
            <EditField
              label="Questions (one per line)"
              rows={3}
              value={arrayToLines(section.questions)}
              onChange={(v) => updateSection(key, "questions", linesToArray(v))}
            />
            <EditField
              label="Purpose"
              rows={2}
              value={section.purpose}
              onChange={(v) => updateSection(key, "purpose", v)}
            />
          </div>
        );
      })}
    ...
    <EditorShell title="Edit Homework">
      ...
      >
        ...
        <EditField
          label="Questions (one per line)"
          rows={3}
          value={arrayToLines(data.questions)}
          onChange={(v) => updateField("questions", linesToArray(v))}
        />
        <EditField
          label="Purpose"
          rows={2}
      ...
      {sections.map((section, i) => (
        ...
        >
          <EditField
            ...
            onChange={(v) => updateSection(i, { ...section, section_title: v })}
          />
            ...
            <div key={j} className="rounded-xl border border-white/10 p-3 space-y-3">
              ...
              {typeof q !== "string" && (
                <EditField
                  ...
                  onChange={(v) => {
                    const questions = (section.questions || []).map((item, idx) =>
                      idx === j && typeof item !== "string" ? { ...item, marks: v } : item,
                    );
                    updateSection(i, { ...section, questions });
                  }}
                />
...
function QuizEditor({ data, onChange }: { data: EditableValue; onChange: EditableChange }) {
  ...
  const shortAnswers = Array.isArray(data.short_answer)
    ? (data.short_answer as QuestionData[])
    : [];
      ...
      {mcqs.map((q, i) => (
        ...
        >
          <EditField
            ...
            rows={3}
            value={q.question}
            onChange={(v) => updateQuestion(i, { ...q, question: v })}
          />
          <div className="grid sm:grid-cols-2 gap-3">
            {Object.entries(q.options || {}).map(([option, value]) => (
              <EditField
                ...
                value={String(value)}
                onChange={(v) =>
                  updateQuestion(i, { ...q, options: { ...(q.options || {}), [option]: v } })
                }
              />
            ))}
          </div>
          <EditField
            label="Marks"
            value={q.marks}
            onChange={(v) => updateQuestion(i, { ...q, marks: v })}
          />
        </div>
      ))}
          ...
          {shortAnswers.map((q, i) => (
            <div key={i} className="rounded-xl border border-white/10 p-3 space-y-3">
              <EditField
                ...
                rows={3}
                value={q.question}
                onChange={(v) => updateShortAnswer(i, { ...q, question: v })}
              />
              <div className="grid sm:grid-cols-2 gap-3">
                <EditField
                  label="Marks"
                  value={q.marks}
                  onChange={(v) => updateShortAnswer(i, { ...q, marks: v })}
                />
              </div>
            </div>
          ))}
...
function AnswerKeyEditor({ data, onChange }: { data: EditableValue; onChange: EditableChange }) {
  const worksheetAnswers = Array.isArray(data.worksheet_answers)
    ? (data.worksheet_answers as AnswerData[])
    : [];
  const quizAnswers = Array.isArray(data.quiz_answers) ? (data.quiz_answers as AnswerData[]) : [];
  const gradingRubric = data.grading_rubric as
    | { grade_bands?: RubricBand[] }
    | RubricBand[]
    | undefined;
  const rubric = (Array.isArray(gradingRubric) ? gradingRubric : gradingRubric?.grade_bands) || [];

  const updateAnswer = (
    kind: "worksheet_answers" | "quiz_answers",
    index: number,
    next: AnswerData,
  ) => {
  ...
  return (
    <EditorShell title="Edit Answer Key">
          ...
          {rubric.map((band, i) => (
            <div key={i} className="grid sm:grid-cols-3 gap-3">
              <EditField
                label="Grade"
                value={band.grade}
                onChange={(v) => updateRubric(i, { ...band, grade: v })}
              />
              <EditField
                label="Range"
                value={band.range}
                onChange={(v) => updateRubric(i, { ...band, range: v })}
              />
              <EditField
                label="Descriptor"
                value={band.descriptor}
                onChange={(v) => updateRubric(i, { ...band, descriptor: v })}
              />
            </div>
          ))}
        </div>
      ...
      <AnswerEditorGroup
        ...
        onChange={(i, next) => updateAnswer("worksheet_answers", i, next)}
      />
      <AnswerEditorGroup
        title="Quiz Answers"
        answers={quizAnswers}
        onChange={(i, next) => updateAnswer("quiz_answers", i, next)}
      />
    </EditorShell>
  );
}

function AnswerEditorGroup({
  title,
  answers,
  onChange,
}: {
  ...
  return (
    ...
    >
      ...
      {answers.map((answer, i) => (
        <div key={i} className="rounded-xl border border-white/10 p-3 space-y-3">
          <EditField
            label={`Answer ${answer.q_number || i + 1}`}
            rows={2}
            value={String(answer.answer ?? "")}
            onChange={(v) => onChange(i, { ...answer, answer: v })}
          />
          <EditField
            label="Explanation"
            rows={2}
            value={answer.explanation}
            onChange={(v) => onChange(i, { ...answer, explanation: v })}
          />
        </div>
      ))}
    </div>
...
function HomeworkView({
  ...
  return (
    <div className="space-y-6">
      ...
      <LessonSectionCard
        section={data}
        label={HOMEWORK_SECTION.label}
        accent={HOMEWORK_SECTION.accent}
        hideTitle
      />
      <Card>
...
function LessonSectionCard({
  ...
  return (
    <Card accent={accent}>
      ...
      {section.description && (
        <p className="text-blue-100 whitespace-pre-wrap mt-2">
          {cleanGeneratedText(section.description)}
        </p>
      )}
      ...
      {Array.isArray(section.questions) && section.questions.length > 0 && (
        <ol className="mt-3 list-decimal list-inside space-y-1 text-blue-100 text-sm">
          {section.questions.map((q: string, j: number) => (
            <li key={j}>{cleanGeneratedText(q)}</li>
          ))}
        </ol>
      )}
      {section.purpose && (
...
const WORKSHEET_GROUPS: {
...
}[] = [
  {
    ...
    match: (question) => Boolean(question.isMcq),
  },
  {
    key: "one_sentence",
    label: "Section B - One Word / One Sentence Answer Questions",
    description: "1 mark each",
    match: (question) => !question.isMcq && marksFor(question) <= 1,
  },
  {
    key: "short",
    label: "Section C - Short Answer Questions",
    description: "2 marks each",
    match: (question) => !question.isMcq && marksFor(question) >= 2 && marksFor(question) < 5,
  },
  {
    key: "long",
    label: "Section D - Long Answer Questions",
    description: "5 marks each",
    match: (question) => !question.isMcq && marksFor(question) >= 5,
  },
] as const;
...
function worksheetMcqs(data?: Worksheet | EditableValue | null): QuestionData[] {
  ...
  if (Array.isArray(data?.multiple_choice_questions))
    return data.multiple_choice_questions as QuestionData[];
  return [];
}
...
function collectWorksheetQuestions(
  data?: Worksheet | EditableValue | null,
...
function descriptorForGrade(rubric: RubricBand[], grade: string, fallback: string) {
  return (
    rubric.find((band) => String(band?.grade || "").toUpperCase() === grade)?.descriptor || fallback
  );
}
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        {total != null && (
          <span className="worksheet-total-pill text-sm px-4 py-1.5 rounded-full text-white font-semibold">
            Total: {total} marks
          </span>
        )}
      </div>
    ...
    <Card>
      <div className="flex flex-wrap items-start justify-between gap-2">
        <p className={`text-white flex-1 ${question.isMcq ? "font-semibold" : ""}`}>
          <span className="font-semibold">Q{question.displayNumber}.</span> {question.question}
        </p>
        ...
        {question.isMcq ? (
          optionEntries(question).map(([key, value]) => (
            <button
              ...
              style={{
                background:
                  studentAnswer === key ? "rgba(59,130,246,0.22)" : "rgba(255,255,255,0.05)",
                border:
                  studentAnswer === key
            ...
            >
              <div
                className={`w-5 h-5 rounded-full border shrink-0 ${studentAnswer === key ? "border-blue-200 bg-blue-400" : "border-white/30"}`}
              />
...
function QuizView({
  ...
  if (!data) return <div className="text-white/60 text-center py-8">Quiz data not available</div>;

  const mcqs = Array.isArray(data.mcq) ? (data.mcq as QuestionData[]) : [];
  const mcqAnswers = getMcqAnswers(answerKey);
  const answerByQuestion = new Map(
    mcqAnswers.map((answer, index) => [
      String(answer.q_number ?? index + 1),
      String(answer.answer).toUpperCase(),
    ]),
  );
  const getQuestionKey = (question: QuestionData, index: number) =>
    String(question.q_number ?? index + 1);
  const getCorrectAnswer = (question: QuestionData, index: number) =>
    answerByQuestion.get(getQuestionKey(question, index)) ??
    answerByQuestion.get(String(index + 1)) ??
    "";
  const answeredCount = mcqs.filter((question, index) =>
    Boolean(selectedAnswers[getQuestionKey(question, index)]),
  ).length;
  ...
  if (mcqs.length === 0) {
    return <div className="text-white/60 text-center py-8">No quiz questions available</div>;
  }
  ...
  return (
    <div className="space-y-6">
                ...
                {Object.entries(q.options || {}).map(([key, value]) => {
                  ...
                  return (
                    <button
                      ...
                      onClick={() =>
                        onSelectAnswer?.({ ...selectedAnswers, [questionKey]: optionKey })
                      }
                    ...
                    >
                      <div
                        className={`w-5 h-5 rounded-full border shrink-0 ${showCorrect ? "border-emerald-300 bg-emerald-400" : showWrong ? "border-red-300 bg-red-400" : isSelected ? "border-blue-200 bg-blue-400" : "border-white/30"}`}
                      />
                      <span className="text-white/70 font-medium w-4">{key}.</span>
                      ...
                      {showWrong && (
                        <span className="text-xs font-semibold text-red-300">Wrong</span>
                      )}
                    </button>
                  );
                })}
      ...
      {!interactive && mcqAnswers.length > 0 && (
        <div>
          <h3 className="text-white font-bold text-lg mb-3">Quiz MCQ Answers</h3>
          <div className="space-y-2">
            {mcqAnswers.map((a, j) => (
              <Card key={j}>
                <p className="flex items-center gap-3">
                  <span className="text-blue-300 font-semibold">Q{a.q_number || j + 1}.</span>
                ...
                {a.explanation && (
                  <p className="italic text-white/60 text-sm mt-2">{a.explanation}</p>
                )}
              </Card>
            ))}
          </div>
...
function QuizResult({ selected, correct }: { selected: string; correct: string }) {
  ...
  return (
    <div
      className={`mt-3 rounded-xl border px-4 py-3 text-sm ${isCorrect ? "border-emerald-400/35 bg-emerald-500/10 text-emerald-200" : "border-red-400/35 bg-red-500/10 text-red-200"}`}
    >
      {isCorrect ? (
        <span className="font-semibold">Correct.</span>
      ) : (
        <span>
          <span className="font-semibold">Wrong.</span> Correct option:{" "}
          <span className="font-bold">{correct || "Not available"}</span>
        </span>
      )}
    </div>
  );
...
function AnswerKeyView({
  ...
  const worksheetAnswerGroups = buildWorksheetAnswerGroups(
    worksheet,
    quiz,
    worksheetAnswers,
    quizAnswers,
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-2">
        ...
        <h2 className="text-white text-2xl font-bold">Answer Key & Grading Rubric</h2>
      </div>
      ...
      {rubric.length > 0 && (
        <Card>
          {totalMarks && (
            <p className="text-blue-300 text-sm font-semibold mb-3">Total Marks: {totalMarks}</p>
          )}
      ...
      {worksheetAnswerGroups.length > 0 && (
        <div>
          <h3 className="text-white font-bold text-lg mb-3">Worksheet Answers</h3>
...
function buildWorksheetAnswerGroups(
  worksheet: Worksheet | undefined,
  quiz: Quiz | undefined,
  worksheetAnswers: AnswerData[],
...
function getMcqAnswers(answerKey?: AnswerKey) {
  ...
  return (quizAnswers as AnswerData[]).filter(
    (a) =>
...
function findAnswerForQuestion(question: QuestionData, answers: AnswerData[]) {
  const keys = [question.originalNumber, question.q_number, question.displayNumber]
    .filter((v) => v != null)
    .map(String);
  return (
    answers.find((answer) => keys.includes(String(answer.q_number))) ??
    answers[Number(question.displayNumber) - 1]
  );
}
...
function sectionMatchesGroup(
  section: WorksheetSectionData,
  groupKey: WorksheetQuestionGroup["key"],
) {
  const title = String(section.section_title || section.title || "").toLowerCase();
  if (groupKey === "one_sentence")
    return (
      title.includes("section b") || title.includes("one word") || title.includes("one sentence")
    );
...
function replaceWorksheetAnswer(
  answerKey: AnswerKey,
  target: QuestionData,
  freshAnswer: AnswerData,
): AnswerKey {
  ...
  const targetKeys = [target.originalNumber, target.q_number, target.displayNumber]
    .filter((v) => v != null)
    .map(String);
  ...
  next.worksheet_answers = answers.map((answer, index) =>
    index === indexToReplace ? replacement : answer,
  );
    ...
    <div className="print-doc text-black">
      ...
      {shouldPrint("quiz") && (
        <PrintPage title="Quiz" pageBreakBefore={isFullKit}>
          <PrintQuiz
            data={lesson.quiz}
            answerKey={includeAnswerKey ? lesson.answer_key : undefined}
          />
        </PrintPage>
      )}
...
function PrintLessonSection({ section, label }: { section: LessonPlanSectionData; label: string }) {
  return (
    <PrintBlock title={`${label}${section.duration ? ` (${section.duration})` : ""}`}>
      {section.activity && (
        <p className="whitespace-pre-wrap">{cleanGeneratedText(section.activity)}</p>
      )}
      {section.content && (
        <p className="whitespace-pre-wrap mt-2">{cleanGeneratedText(section.content)}</p>
      )}
      {section.description && (
        <p className="whitespace-pre-wrap mt-2">{cleanGeneratedText(section.description)}</p>
      )}
      {Array.isArray(section.key_terms) && section.key_terms.length > 0 && (
        <p className="mt-2">
          <strong>Key terms:</strong> {section.key_terms.map(cleanGeneratedText).join(", ")}
        </p>
      )}
      {Array.isArray(section.materials) && section.materials.length > 0 && (
...
function PrintWorksheet({ data }: { data: Worksheet }) {
  ...
  return (
    <div>
      {data.instructions && (
        <PrintBlock>
          <p className="italic">{data.instructions}</p>
        </PrintBlock>
      )}
      {groupedQuestions.map((group) => (
...
function PrintWorksheetQuestion({ question }: { question: QuestionData }) {
  return (
    <PrintQuestion
      number={question.displayNumber ?? question.q_number ?? ""}
      marks={question.marks}
      options={question.isMcq ? question.options : undefined}
    >
      {question.question}
    </PrintQuestion>
  );
  ...
  return (
    <div>
      ...
      {mcqs.map((q, j) => (
        <PrintBlock key={j}>
          <p>
            <strong>Q{q.q_number || j + 1}.</strong> {q.question}
          </p>
      ...
      {options && Object.keys(options).length > 0 ? (
        ...
        <div className="mt-3 space-y-3">
          {Array.from({ length: lineCount }).map((_, line) => (
            <div key={line} className="border-b border-slate-300 h-6" />
          ))}
        </div>
      )}
    </div>
