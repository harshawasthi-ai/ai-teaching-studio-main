...
export async function saveLessonSection(lessonId: string, userId: string, section: LessonSection, value: unknown) {
...
export async function saveLessonSections(lessonId: string, userId: string, values: Partial<Record<LessonSection, unknown>>) {
  const { error } = await supabase
    .from('lessons')
    .update(values)
    .eq('id', lessonId)
...
export async function regenerateWorksheetAnswer(payload: {
  metadata: LessonKit['metadata']
  question: unknown
  current_answer?: unknown
  answer_key_instruction?: string
}, userId?: string): Promise<RegeneratedAnswer> {
  ...
  const text = await response.text()
  if (!response.ok) throw new Error(text || `Answer regeneration failed with status ${response.status}`)
  const data = parseWebhookResponse(text, 'Server returned an invalid answer regeneration response')
  if (!data.success) throw new Error(data.error || 'Answer regeneration failed')

  const answerData =
    data.data?.answer_key_item ||
    data.data?.data ||
    data.data ||
    data.answer_key_item ||
    data
  const answer =
    answerData?.answer ??
  ...
  return {
    answer: answer.trim(),
    explanation:
      typeof answerData?.explanation === 'string' ? answerData.explanation.trim() :
      typeof answerData?.data?.explanation === 'string' ? answerData.data.explanation.trim() :
      typeof answerData?.output?.explanation === 'string' ? answerData.output.explanation.trim() :
      undefined,
  }
}
...
export async function evaluateHomeworkWithAi(payload: {
  ...
  const text = await response.text()
  if (!response.ok) throw new Error(text || `Homework evaluation failed with status ${response.status}`)
  const data = parseWebhookResponse(text, 'Server returned an invalid homework evaluation response')
